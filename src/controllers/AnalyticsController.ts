...
export default {
  ...
  async getAnalyticsActivity(request: Request, response: Response) {
    ...
    try {
      ...
      const inveniraStdIDs = await AppDataSource.createQueryBuilder()
      ...
      const analytics = await Promise.all(inveniraStdIDs.map(async ({ inveniraStdID }) => {
...
//     ],
//     "qualAnalytics": [
//       { "name": "Analytic 3", "type": "text/plain", "value": "texto" },
//       { "name": "Analytic 4", "type": "URL", "value": "http://<endereço>/1001" }
//   },
// ]
